...
class MigrationsController extends Controller {
  ...
  async runMigrations(req, res) {
    try {
      ...
      res.status(200).json({ mensagem: "Migrações executadas com sucesso.", detalhes: result });
    } catch (error) {
      console.error(error.message);
      res.status(500).json({ mensagem: "Erro ao executar migrações.", erro: error.message });
    }
  }
}
