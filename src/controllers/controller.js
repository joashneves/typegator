class Controller {
  ...
  async pegaTodos(req, res) {
    try {
      const listaDeRegistro =
        await this.entidadeServices.pegaTodosOsRegistros();
  ...
  async criaNovo(req, res) {
    ...
    try {
      const foiCriadoDados =
        await this.entidadeServices.criaRegistro(dadosCriado);
      ...
      return res
        .status(500)
        .json({ error: "ocorreu um erro", messagem: error.messagem });
    }
  }
  ...
  async atualiza(req, res) {
    ...
    try {
      const foiAtualizado = await this.entidadeServices.atualizaRegistro(
        dadosAtualizados,
        Number(id),
      );
      if (!foiAtualizado) {
